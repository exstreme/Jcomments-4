	...
	public static function addComment($values = array())
	{
		...
		if ($acl->canComment())
		{
			...
			if (($acl->check('floodprotection') == 1) && (JCommentsSecurity::checkFlood($userIP)))
			{
				...
				&& ($acl->check('enable_permission_show_checkbox_terms_of_use') == 1))
			{    // Is checkbox enabled from backend (tab Permissions)?
		...
		if ($hash === md5($app->get('secret')))
		{
			...
			if ($step == 0)
			{
				if ($app->get('caching') != 0)
				{
					...
					foreach ($rows as $row)
					{
						$cache = Factory::getCache('com_jcomments_objects_' . strtolower($row));
						$cache->clean();
					}
				}
			...
			if ($total > 0)
			{
				...
				$query->order($db->quoteName(array('c.object_group', 'c.lang')));
