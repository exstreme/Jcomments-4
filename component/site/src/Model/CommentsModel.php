...
class CommentsModel extends ListModel
{
	...
	protected function getCommentsQuery()
	{
				...
				$this->getState(
					...
					array_diff(
						array(
							...
							$db->quoteName('c.pinned'),
							'(' . $db->quoteName('c.isgood') . ' - ' . $db->quoteName('c.ispoor') . ')'
								. ' AS ' . $db->quoteName('vote_value')
						),
						$excludeFields
					)
				)
		...
		$query->select($db->quoteName('usr_c.name', 'editor') . ',' . $db->quoteName('usr_c.username', 'editor_username'))
		...
		// NOTE! In tree mode, unpublishing the parent comment will make the child comments invisible. Even if they are published.
		if ($published !== null)
		{
